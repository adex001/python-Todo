...
class TodoResourceTestCase(TestCase):
    ...
    def test_to_retrieve_a_single_todo(self):
        ...
        response = self.client.get('/api/v1/to-do/1')
        expected_response = 'Workout tomorrow'

        assert response.status_code == 200
        self.assertEqual(response.json.get('todo').get('id'), 1)
    ...
    def test_to_retrieve_a_single_todo_that_does_not_exist(self):
        ''' Retrieve a specific todos '''
        response = self.client.get('/api/v1/to-do/6')
    ...
    def test_to_update_a_single_todo(self):
        ...
        response = self.client.patch('/api/v1/to-do/1', data = json.dumps(todo_object), headers=self.headers)
    ...
    def test_to_update_an_invalid_todo(self):
        ...
        response = self.client.patch('/api/v1/to-do/6')
        expected_response = 'todo not found'
        assert response.status_code == 404
        self.assertEqual(response.json.get('error'), expected_response)
    ...
    def test_to_create_a_todo(self):
        ...
        self.assertEqual(response.json.get('todo').get('id'), expected_response['id'])
    ...
    def test_to_delete_a_todo(self):
        ...
        response = self.client.delete('/api/v1/to-do/3')
        assert response.status_code == 200
